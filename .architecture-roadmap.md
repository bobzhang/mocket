...
- **[P1]** Phase 4 prerequisite was stale. Current `main` already has `Mocket::ws` with private `ws_static_routes` / `ws_dynamic_routes` and `serve_async.mbt` reads those fields directly. Option A (pub(r) field) / Option B (register method) is not the real issue. Phase 4 keeps the existing `Mocket::ws` method and private storage at root; the extraction moves only the types and runtime.
...
| **3** | `websocket/` | websocket.mbt + websocket_async.mbt + wbtest, ~2500 LOC. `Mocket::ws` stays at root; only types + runtime move. | medium | §Phase 3 below |
...
- `server/` for `serve_async.mbt` and `NativeServeOptions` — serve is the heart of the framework and calls into every other root type; extracting it creates cycles with middleware and requires exposing `Mocket` internals as `pub`. Not worth the churn.
