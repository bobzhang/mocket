...
- **[P1]** Phase 3 (`static.mbt` merge) was mis-scoped. Root `static.mbt` owns the abstraction (`StaticAssetMeta`, `ServeStaticProvider`, `StaticResolvedResponder`, `Mocket::static_assets`); `static_file/` is only one provider against that trait. This is **not** legacy wrapper cleanup. Phase 3 is deferred until a bigger rethink of the static-files abstraction layer.
- **[P1]** Phase 4 prerequisite was stale. Current `main` already has `Mocket::ws` with private `ws_static_routes` / `ws_dynamic_routes` and `serve_async.native.mbt` reads those fields directly. Option A (pub(r) field) / Option B (register method) is not the real issue. Phase 4 keeps the existing `Mocket::ws` method and private storage at root; the extraction moves only the types and runtime.
...
2. **Preserve method coherence** — MoonBit disallows defining methods on a type from a different package than the type's definition. Any cluster that owns a method on `Mocket` or `MocketEvent` must either stay at root or leave its method accessor at root (see `cors/` precedent, confirmed again for Phase 1's `MocketEvent::request_id`).
...
| **3** | `websocket/` | websocket.mbt + websocket_async.native.mbt + wbtest, ~2500 LOC. `Mocket::ws` stays at root; only types + runtime move. | medium | §Phase 3 below |
...
- `testing/` for TestClient — circular dep (TestClient → Mocket; root tests → TestClient). Idiomatic MoonBit testing keeps test harness with the type it dispatches on. Leave at root.
- `typed_handlers/` for `Mocket::get/post/put/...` convenience methods and `wrap_error_handler` — method coherence rule binds them to root. Moving the helper functions alone while leaving the methods at root makes the API incoherent.
- `server/` for `serve_async.native.mbt` and `NativeServeOptions` — serve is the heart of the framework and calls into every other root type; extracting it creates cycles with middleware and requires exposing `Mocket` internals as `pub`. Not worth the churn.
- `fetch/` consolidation — the existing `fetch/` sub-package already owns the high-level API; the root `fetch.mbt` / `fetch.native.mbt` hold the FFI enums (`FetchCredentials`, `FetchMode`) and the low-level native binding, which `fetch/` consumes. No change needed; verify the split during Phase 3 housekeeping.
...
// After
pub type Middleware = async (MocketEvent, MiddlewareNext) -> &@responder.Responder noraise
```
...
### Goal
Extract `websocket.mbt` (types: `WebSocketPeer`, `WebSocketEvent`, `WebSocketAggregatedMessage`, `WebSocketHandler` type alias) and `websocket_async.native.mbt` (native runtime: `ws_send`, `ws_subscribe`, `ws_publish`, etc.) into a new `bobzhang/crescent/websocket` sub-package.

### Constraint discovered by codex review
- `Mocket::ws` on current `main` already registers handlers into private `ws_static_routes` / `ws_dynamic_routes` radix storage on `Mocket` ([index.mbt](/Users/hongbozhang/git/crescent/index.mbt:240)).
- `serve_async.native.mbt` dispatches WebSocket routes by reading those fields directly.
- So **no router API change is needed** — the original "Option A/B prerequisite" in the draft was stale. `Mocket::ws` stays at root (method coherence); only the types and runtime move.

### What moves
- `websocket.mbt` → `websocket/websocket.mbt` (types + handler alias + any free functions)
- `websocket_async.native.mbt` → `websocket/websocket_async.native.mbt` (~496 LOC native runtime)
...
### What stays at root
- `Mocket::ws` method (public, pinned by coherence)
- `ws_static_routes` / `ws_dynamic_routes` fields on `Mocket`
- `serve_async.native.mbt`'s WebSocket upgrade path

### New dependency direction
Root's `index.mbt` (`Mocket` struct) uses `WebSocketHandler` as a field type. After the extraction, that field type is `@websocket.WebSocketHandler`. Root's `moon.pkg` needs `"bobzhang/crescent/websocket" @websocket` in the **main** import block, not just `for "test"`, because it's load-bearing for the `Mocket` struct definition. `websocket/` itself must NOT import root — otherwise cycle.

Codex warning: this works only if `websocket/` is root-independent. If any function in `websocket_async.native.mbt` references `Mocket` or other root types, it has to either (a) move its use-site to root or (b) take the root type as a parameter rather than importing it. Review `websocket_async.native.mbt` carefully during implementation.

### Breaking API changes
```
// Type alias in Mocket field
...
### Examples / benchmarks
`examples/` and `benchmarks/` are separate modules per `moon.mod.json`. Each phase updates the relevant example's `moon.pkg` imports and source call sites. Track this per phase — don't batch.
